...
Live-editor cleanup waits for three seconds after the latest edit, giving the user time to move the caret back to a newly
inserted empty paragraph or list item. When the delay expires, Jot keeps the eligible empty line under a collapsed caret
and normalizes the others. The Local Draft and Google Drive snapshot is still fully canonical during the delay, and a
successful save never replaces the protected live document. Moving the caret away, reloading, or explicitly replacing
the document may remove an empty line normally after the grace period. This behavior can be disabled in **Settings** for
source-preserving workflows. Literal placeholder-looking lines inside fenced or indented code blocks, or raw HTML blocks,
