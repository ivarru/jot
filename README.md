...
The Google sign-in also requests Google Photos Picker access, append-only Library access, and read access to app-created Google Photos media items. Jot can attach images from Google Photos, device upload, camera capture, or the clipboard. It copies the chosen resolution into a Jot-created Google Photos album named `jot`, stores attachment metadata as JSON in Drive `jot/Image Attachments`, and inserts a plain markdown `jot:image:<id>` reference. The editor resolves those references back to Google Photos image previews at runtime.
